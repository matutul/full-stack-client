...
import { initializeFirebaseAppFramework, newUserCreateWithEmailPassword, userSignInWithEmailAndPassword, userSignInWithFacebook, userSignInWithGoogle } from './Firebase';
import './Login.css';
    ...
    const handleSubmit = (e) => {
        ...
        if (isNewUser && user.email && user.password) {
            newUserCreateWithEmailPassword(user)
                .then(res => {
                    console.log("email password sign up response: ", res)
        ...
        if (!isNewUser && user.email && user.password) {
            userSignInWithEmailAndPassword(user)
                .then(res => {
                    console.log("email password login response", res);
                    const responseKey = Object.keys(res);
                    if (responseKey.find(key => key === "displayName")) {
                        setLoggedInUser(res);
            ...
            <div className="auth-card">
                ...
                <form onSubmit={handleSubmit} className="auth-form">
                    <div className="form-checkbox">
                        <input type="checkbox" defaultChecked={isNewUser} onClick={() => setIsNewUser(!isNewUser)} name="signInOrOut" id="input-1" />
                        <label htmlFor="input-1" className="check-label">New user registration</label>
                    </div>

                    {
                        isNewUser && <input className="form-input" type="text" onBlur={handleOnBLur} name="fullname" id="input-2" placeholder="Full name" required />
                    }

                    <input className="form-input" type="text" onBlur={handleOnBLur} name="email" id="input-3" placeholder="Email address" required />
                    <input className="form-input" type="password" onBlur={handleOnBLur} name="password" id="input-5" placeholder="Password" required />
                    {
                        isNewUser && <input className="form-input" type="password" onBlur={handleOnBLur} name="confirmPassword" id="input-6" placeholder="Confirm password" required />
                    }
                    {
